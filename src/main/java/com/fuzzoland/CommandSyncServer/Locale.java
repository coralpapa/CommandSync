...
public class Locale {
    ...
    public void setup() {
        ...
        if (this.saveLocale(loc)) {
            ...
            try (FileReader fr = new FileReader(this.localeFile)) {
            	...
            	Debugger.getInstance().Log(Level.WARNING, "Failed to load " + loc + " locale!", ex);
	...
	private boolean saveLocale(final String name) {
        ...
        File enFile = new File(CSS.getInstance().getDataFolder() + File.separator + "en_US.properties");
        ...
        if (getClass().getResource(is) == null) {
            ...
            if (!enFile.exists()){
	            ...
	            try {
	    			...
	    			Files.copy(jarFS.getPath(u.toString().split("!")[1]), new File(CSS.getInstance().getDataFolder() + File.separator + "en_US.properties").toPath());
	    	        jarFS.close();
	        ...
	        try {
	            ...
	            FileSystem jarFS = FileSystems.newFileSystem(URI.create(u.toString().split("!")[0]), new HashMap<>());
				Files.copy(jarFS.getPath(u.toString().split("!")[1]), new File(CSS.getInstance().getDataFolder() + File.separator + name + ".properties").toPath());
		        jarFS.close();
